...
public class Odometer extends OdometerData implements Runnable {
	...
	private Odometer(EV3LargeRegulatedMotor leftMotor, EV3LargeRegulatedMotor rightMotor, final double TRACK,
		...
		this.theta = 0.0;
		this.rad = 0.0;
		this.Radius = Lab2.WHEEL_RAD;
		this.width = Lab2.TRACK;
	...
	public void run() {
		...
		while (true) {
			...
			deltaD = 0.5 * (distL + distR); // compute vehicle displacement using average value of summation of both wheels
			deltaT = (distL - distR) / TR; // compute change in heading direction
			Theta += deltaT; // update heading direction
			dX = deltaD * Math.sin(Theta); // compute changes of X component of displacement
			dY = deltaD * Math.cos(Theta); // compute changes of Y component of displacement
